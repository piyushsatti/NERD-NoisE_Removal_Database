...
%     tmp1(d,10)=psnr(Img,fahpf_OutImg);                    % Calculate PSNR of   
%     eval(['psnr_fahpf' num2str(d) '= tmp1(d,10)']);
%     tmp2(d,10)=ssim(Img,fahpf_OutImg);                    % Calculate SSIM
%     eval(['ssim_fahpf' num2str(d) '= tmp2(d,10)']);
%     filename = ['fahpf' num2str(d) '.jpg'];
%     imwrite(fahpf_OutImg,filename);

%     f = @() fahpf3(nImg);
%     tmp3(d,11)= timeit(f);
%     fahpf3_OutImg = fahpf3(nImg);                           % Call  median filter Proposed-II
%     tmp1(d,11)=psnr(Img,fahpf3_OutImg);                    % Calculate PSNR of fahpf3  
%     eval(['psnr_fahpf3' num2str(d) '= tmp1(d,11)']);
%     tmp2(d,11)=ssim(Img,fahpf3_OutImg);                    % Calculate SSIM
%     eval(['ssim_fahpf3' num2str(d) '= tmp2(d,11)']);
%     filename = ['Prop' num2str(d) '.jpg'];
%     imwrite(fahpf3_OutImg,filename);

f = @() fahpf3_stage1(nImg);
    tmp3(d,12)= timeit(f);
f = @() fahpf3_stage2(nImg);
    tmp3(d,13)= timeit(f);
f = @() fahpf3_stage3(nImg);
    tmp3(d,14)= timeit(f);
f = @() fahpf3(nImg);
    tmp3(d,15)= timeit(f);
end
