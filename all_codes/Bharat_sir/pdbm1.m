...
for i= 1+pad: row+pad
    for j= 1+pad: col+pad
        if((imgZP(i,j)==0)||(imgZP(i,j)==255)) % Check pixel is noisy
            tmp= sort(sort(imgZP(i-rng:i+rng,j-rng:j+rng)));% Row & Col sorting
            tm= sort([tmp(1,1),tmp(2,2),tmp(3,3)]);         % Diagonal sorting
            if((tm(1,2)==0)||(tm(1,2)==255)) % Check pixel after PM is noisy 
%             if((tmp(1+rng,1+rng)==0)||(tmp(1+rng,1+rng)==255)) % Check pixel after PM is noisy
                ...
                tmp1(tmp1==255)=[];
                pixel1= median(tmp1);
                if((pixel1==0)||(pixel1==255)) % Check pixel is noisy
                    tmp3= sort(sort(imZP(i-2*rng:i+2*rng,j-2*rng:j+2*rng)));
                    if((imgZP(i,j)==0)||(imgZP(i,j)==255))    % Check pixel after tm is noisy 
