...
function OutImg = Propmf(nImg)
% clc; clear;
% Img= imread('lena_16.tif');   % Reading input image
% d=0.5;   % Noise density
...
imgZP= double(zeros(row+2,col+2));
...
for i= 1: row+2
    for j= 1: col+2
        ...
        elseif(i==1)
            imgZP(i,j)= (nImg(i,j-1)+nImg(i+1,j-1))/2;
        elseif(i==row+2)
            imgZP(i,j)= (nImg(i-2,j-1)+nImg(i-3,j-1))/2;
        elseif(j==1)
            imgZP(i,j)= (nImg(i-1,j)+nImg(i-1,j+1))/2;
        elseif(j==col+2)
            imgZP(i,j)= (nImg(i-1,j-2)+nImg(i-1,j-3))/2;
        else
